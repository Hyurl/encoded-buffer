...
function throwError() {
    throw new TypeError("The buffer cannot be destructed.");
}
    ...
    switch (type) {
        case "Array":
            ...
            while (data.byteLength > 0) {
                let _part = destructPart(getPart(data));
                data = _part.left;
                res.push(_part.data);
            }
            break;
...
function destruct(buf) {
    try {
        let part = destructPart(getPart(buf));
        let res = [part.data];
        let { left } = part;
        while (left.byteLength > 0) {
            let _part = destructPart(getPart(left));
            res.push(_part.data);
            left = _part.left;
        }
        return res;
